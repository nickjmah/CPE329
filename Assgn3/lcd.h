 ...
 *
 * This file contains function prototypes and macros for everything needed to define the lcd
 * All commands sent are in 4-bit mode, with the exception of halfBitInit()
 *
 * \author Nick Mah
 * \author Jason Zhou
 *
 ...
 *
 *  This function sends all of the commands required to initialize the lcd in 4-bit mode
 ...
 *
 * This function calls writeData for each character in the string. The function takes a
 * char array pointer as an argument
 *
 ...
 *
 *  This function sends a 4-bit mode command to the LCD. The function takes in a char array
 *  pointer and iterates through the string until the char value is a null character
 *
 ...
 * \bug This doesn't work yet. Do not use. This is because of readData() not working
 */
uint8_t checkBusy();

/** \brief Writes a character to the display
 *
 * This set the R/S bit high, then sends the data through the writeCommand() with the data.
 * Then, the R/S bit is set back low
 *
